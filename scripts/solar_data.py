...
class SolarSite(object):
    ...
    def get_nsrdb_data(self,year,leap_year,interval,utc):
        ...
        if not api_key:
            raise RuntimeError('NREL_API_KEY environment variable is not set. Set it before running the SoDa generator.')
        ...
        # Declare url string
        url = 'https://developer.nlr.gov/api/nsrdb/v2/solar/himawari-download.csv?wkt=POINT({lon}%20{lat})&names={year}&leap_day={leap}&interval={interval}&utc={utc}&full_name={name}&email={email}&affiliation={affiliation}&mailing_list={mailing_list}&reason={reason}&api_key={api}&attributes={attr}'.format(year=year, lat=self.lat, lon=self.lon, leap=str(leap_year).lower(), interval=interval, utc=str(utc).lower(), name=your_name, email=your_email, mailing_list=mailing_list, affiliation=your_affiliation, reason=reason_for_use, api=api_key, attr=attributes)\
    ...
    def generate_solar_power_from_nsrdb(self,clearsky,capacity,DC_AC_ratio,tilt,azimuth,inv_eff,losses,array_type,year = None,leap_year = None,interval = None,utc = None):
        ...
        ssc.data_set_array(wfd, 'year'.encode('utf-8'), self.resource_data.Year.to_numpy())
        ssc.data_set_array(wfd, 'month'.encode('utf-8'), self.resource_data.Month.to_numpy())
        ssc.data_set_array(wfd, 'day'.encode('utf-8'), self.resource_data.Day.to_numpy())
        ssc.data_set_array(wfd, 'hour'.encode('utf-8'), self.resource_data.Hour.to_numpy())
        ssc.data_set_array(wfd, 'minute'.encode('utf-8'), self.resource_data.Minute.to_numpy())

        ssc.data_set_array(wfd, 'dn'.encode('utf-8'),
            self.resource_data["{}DNI".format(clearsky_str)].to_numpy())

        ssc.data_set_array(wfd, 'df'.encode('utf-8'),
            self.resource_data["{}DHI".format(clearsky_str)].to_numpy())

        ssc.data_set_array(wfd, 'wspd'.encode('utf-8'),
            self.resource_data['Wind Speed'].to_numpy())

        ssc.data_set_array(wfd, 'tdry'.encode('utf-8'),
            self.resource_data.Temperature.to_numpy())
    ...
    def generate_high_resolution_power_data(self, resolution, date):
        ...
        ts = self.solar_power_from_nsrdb.loc[date : date + " 23:59:59"].resample("1s").interpolate(method="linear")
        ts = ts.reindex(date_index2).fillna(0)

        ts *= (7.5/self.capacity)

        ct = self.cloud_type.loc[date : date + " 23:59:59"].resample("1s").ffill()
        ...
        for i in range(0,len(zm)):
            if randm[i] > 1 - pm[df["CloudType"].iloc[i]]:
                bm[i] = 1
            if randm[i] > 1 - pw[df["CloudType"].iloc[i]]:
                bw[i] = 1
