...
PSO comparison script for thesis BESS sizing across all ramp-rate scenarios.
...
- Use this PSO script as a comparison/validation method for R20, R10, R5, and R3. R5 can still be treated as the main baseline in the thesis narrative.
- Compare PSO result against:
  bess_sizing_outputs/03_bess_sizing_ramp_scenarios.csv

Default use
-----------
python 05_bess_pso_all_ramp_scenarios.py \
    --input soda_final_outputs/synthetic_pv_soda_2020_FINAL.csv \
    --scenario R5

Optional all-scenario run
-------------------------
python 05_bess_pso_all_ramp_scenarios.py --scenario ALL
...
2. Continuous debug mode: candidate P_BESS and E_BESS are NOT rounded to the grid-search resolution.
   This version is intended to test stochastic PSO behavior against deterministic grid search, not to replace the main thesis script.
3. The objective function applies large penalties to infeasible candidates.
4. When --scenario ALL is used, the same PSO settings are applied to R20, R10, R5, and R3.
5. The economic parameters below are aligned with the latest thesis/PPT values.
...
DEFAULT_N_ITER = 60
DEFAULT_SEED = 42
ROUND_STEP = None  # Continuous debug mode: no MW/MWh rounding
...
def round_to_step(x: float, step: float | None = ROUND_STEP) -> float:
    """Return x unchanged in continuous debug mode; otherwise round to a fixed step."""
    if step is None:
        return float(x)
...
def repair_candidate(p: float, e: float) -> tuple[float, float]:
    ...
    # Optional rounding after repair. In this continuous debug version, ROUND_STEP=None, so values are unchanged.
    p = round_to_step(p)
    e = round_to_step(e)

    # A second repair after optional rounding.
...
def run_pso_for_scenario(
    ...
    for it in range(n_iter):
        ...
        print(
            ...
            f"P={gbest[0]:.4f} MW | E={gbest[1]:.4f} MWh | "
...
def main() -> None:
    ...
    parser.add_argument("--same-seed-all", action="store_true", help="Use exactly the same seed for every scenario. By default, scenario runs use seed+scenario_index for traceability.")
    ...
    print(f"Particles: {args.particles} | Iterations: {args.iters} | Base seed: {args.seed}")
    ...
    for scen_idx, scen in enumerate(scenarios):
        scenario_seed = args.seed if args.same_seed_all else args.seed + scen_idx
        print("\n" + "=" * 72)
        print(f"Running PSO for {scen} | seed={scenario_seed}")
        print("=" * 72)
        ...
        results.append(res)
        print(
            f"\nFinal PSO {scen}: P={res.p_bess_mw:.4f} MW | E={res.e_bess_mwh:.4f} MWh | "
